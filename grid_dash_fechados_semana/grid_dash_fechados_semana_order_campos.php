<?php
   ...
   if (!function_exists("NM_is_utf8"))
   {
    ...
    $Ord_Cmp = new grid_dash_fechados_semana_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_dash_fechados_semana_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['qtde_dia'] = "qtde_dia";
       $tab_converte["qtde_dia"]   = "qtde_dia";
   }
   else
   {
       $tab_def_campos['qtde_dia'] = "COUNT(*)";
       $tab_converte["COUNT(*)"]   = "qtde_dia";
   }
   $tab_labels["qtde_dia"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['labels']["qtde_dia"])) ? $_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['labels']["qtde_dia"] : "Qtde Dia";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_dash_fechados_semana_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_dash_fechados_semana']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
