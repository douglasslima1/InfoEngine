...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_cli_end"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_form_cli_end_submit_form(y, z); 
} 
<?php
}
?>
...
function cep_cep_(num_cep, campos_cep)
{
   retorno_cep  = campos_cep;
   jsrsExecute("form_cli_end_cep.php?cep=" + num_cep + "&onchange=s", recebe_cep_, "cep_", num_cep, false);
}
function recebe_cep_(string)
{
...
{
   nm_field_disabled("endereco_=disabled;bairro_=disabled;municipio_=disabled;idestado_=disabled", "");
