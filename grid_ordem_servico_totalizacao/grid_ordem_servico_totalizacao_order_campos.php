<?php
   ...
   if (!function_exists("NM_is_utf8"))
   {
    ...
    $Ord_Cmp = new grid_ordem_servico_totalizacao_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_ordem_servico_totalizacao_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   {
       $tab_def_campos['mes'] = "CONCAT(MONTH(dataAbertura),'/',YEAR(dataAbertura))";
       $tab_converte["CONCAT(MONTH(dataAbertura),'/',YEAR(dataAbertura))"]   = "mes";
   }
   $tab_labels["mes"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['labels']["mes"])) ? $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['labels']["mes"] : "Mes";
   $tab_ger_campos['preco'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['preco'] = "preco";
       $tab_converte["preco"]   = "preco";
   }
   else
   {
       $tab_def_campos['preco'] = "SUM(preco)";
       $tab_converte["SUM(preco)"]   = "preco";
   }
   $tab_labels["preco"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['labels']["preco"])) ? $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['labels']["preco"] : "Preco";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_ordem_servico_totalizacao_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_ordem_servico_totalizacao']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
